...
export function formatTime(array: any[], elId: string) {
  ...
  if (element) element.textContent = form;
};

export function buildRows(data: any[]) {
...
export function sortedCount<T extends { artist: string }>(array: T[]): Record<string, number> {
  const artistCounts: Record<string, number> = array.reduce((acc, item) => {
